"""Real Windows proxy before/after short-path repair; never start or edit the real game."""
import ctypes
import hashlib
import json
import re
...
with tempfile.TemporaryDirectory(prefix='GuiguLaunchRepair-') as directory:
    ...
    assert game.is_dir() and not game.is_junction()
    ...
    result = {'result': 'passed', 'windows_ansi_code_page': 1252,
              ...
              'checks': ['exact bundled proxy', 'original failure reproduced', 'Windows short path without moving files',
                         'original directory preserved', 'manifest-derived launch reaches main after repair',
    ...
    (ROOT / 'evidence/launch-short-path-native.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
