...
try:
    ...
    assert app.action.cget('text')=='Translate with DeepSeek'
    ImageGrab.grab(window=app.winfo_id()).save(APP/'projects/simple-preview.png')
    fake={'state':'success','count':3,'pending':0,'folder':str(APP/'projects/2859071194')}
    with patch('desktop.run_job',return_value=fake):
        app.translate()
        deadline=time.monotonic()+5
        while app.busy and time.monotonic()<deadline:
            app.update();time.sleep(0.02)
    assert app.status.get().startswith('Success')
